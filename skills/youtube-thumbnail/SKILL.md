...
```bash
.venv/bin/python3 .claude/skills/youtube-thumbnail/scripts/search_examples.py \
  --query "{video topic}" \
  --top 5 \
  --min-views 10000
```
...
```bash
.venv/bin/python3 .claude/skills/youtube-thumbnail/scripts/generate_thumbnail.py \
...
**For 5+ concepts in a single session, do NOT run in parallel.** Fire SEQUENTIALLY in a single bash loop with at least 20s `sleep` between calls. Parallel-burst on the preview image model (`gemini-3-pro-image`) trips per-key rate-limiting that surfaces as cascading `503 UNAVAILABLE` errors — staggered retries don't help once the burst window is tripped. See `feedback_gemini_image_parallel.md`.

**Before assuming the model is overloaded on a 503:** verify the `GEMINI_API_KEY` in `.env` matches a key that actually exists in https://aistudio.google.com/apikey. A deleted/rotated key returns `400 INVALID_ARGUMENT — API key expired` which looks indistinguishable from overload at a glance. Check the key suffix against the dashboard before queueing retries.
...
```bash
.venv/bin/python3 .claude/skills/youtube-thumbnail/scripts/combine_thumbnails.py \
...
- Bright OR dark are both valid — it depends on what the thumbnail is showcasing. Don't treat either as mandatory; Austin's best thumbnails span both a clean high-key look (soft light-blue / warm) and a dark cinematic glowing look. Match the direction of his uploaded example thumbnails for the specific video.
...
| `429 RESOURCE_EXHAUSTED` "monthly spending cap" | Distinct from 503-overload and the 403-playbook — the AI Studio project hit its monthly spend cap. Fix: Austin raises it at https://ai.studio/spend (30 sec). Nothing on our end unblocks it; do NOT burst-retry. Render cost is ~4¢/image so the cap is usually set low, not actually drained. |
